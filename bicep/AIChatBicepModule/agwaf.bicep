...
var agwEnabledApps = sort(
  filter(items(containerApps), app => app.value.?agw_enabled ?? false),
  (a, b) => (a.value.agw_index ?? 999) < (b.value.agw_index ?? 999)
)

// Filter container apps that have URL rewrite enabled, sorted by rewrite sequence for stable ordering
var containerAppsWithRewrite = sort(
  filter(agwEnabledApps, app => app.value.?agw_rewrite_enabled ?? false),
  (a, b) => (a.value.agw_rewrite_sequence ?? 999) < (b.value.agw_rewrite_sequence ?? 999)
)
...
resource appGateway 'Microsoft.Network/applicationGateways@2024-05-01' = {
  ...
  properties: {
    ...
    urlPathMaps: [
      {
        ...
        properties: {
          ...
          pathRules: concat(containerAppPathRules, investmentPathRules)
        }
      }
    ]
    ...
    probes: concat(investmentProbes, containerAppProbes)
